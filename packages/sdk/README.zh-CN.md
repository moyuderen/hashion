...
> **依赖：** 需要安装 `spark-md5`（`npm i spark-md5`）。如果未安装，运行时会报错并提示安装方式。
...
在 Web Worker 中计算 MD5，MD5 算法已**内置**，**无需额外依赖**。适合大文件场景，可以减少主线程阻塞，让页面交互更流畅。
...
| 实现 | 算法 | 线程 | 额外依赖 | 适用场景 |
| --- | --- | --- | --- | --- |
| `Spark` | MD5 | 主线程 | `spark-md5` | 简单 MD5 计算场景 |
| `SparkWorker` | MD5 | Web Worker | 无（内置） | 大文件计算、希望页面更流畅的场景 |
| `Sha` | SHA-1 / SHA-256 / SHA-384 / SHA-512 | 主线程 + Web Crypto | 无 | 标准 SHA 算法场景 |

## 注意事项

- `Spark` 需要安装 `spark-md5`（`npm i spark-md5`）。如果未安装，运行时会报错并提示安装方式。
